         ...
         (
   				SELECT
   				          PM.FIR,
   				          Order_By_Tip
   				        , case when pm.fir= 'RUBIN' and ORDER_BY_TIP=1 AND (GOTOV_PR_GPR = 323 or instr(upper(gotov_pr_PUN_naziv),'IZVOZ')>0 )  Then
   				               2
   				          ELSE
   				               Order_By_TIP_V
   				          end Order_By_TIP_V,
                ...
                (
					Select vrsta
					     ...
					     , CASE WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=1 THEN
					                 'INVEJ'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=2 THEN
					                 'MONUS'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=3 THEN
					                 'RUBIN'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=4 THEN
					                 'VITAL'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=5 THEN
					                 'ALBUS'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=6 THEN
					                 'MBS'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=7 THEN
					                 'PEKARA'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=11 THEN
					                 'SUNCE'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=12 THEN
					                 'RATAR'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=13 THEN
					                 'LUKAP'
					            WHEN (SELECT UL02 FROM MAPIRANJE WHERE MODUL='ID_FIRME')=14 THEN
					                 'MEDELA'
