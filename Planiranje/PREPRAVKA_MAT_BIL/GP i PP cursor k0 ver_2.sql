...
(
	...
	, (select sum(stanje) from ZAL_GOD_MAG_DAT
	   where pro=plan.GOTOV_PR_SIFRA
	     and na_dat=&dDatDo
	     and mag in (select vrednost from PLANIRANJE_MAPIRANJE  where VRSTA ='MAGACINI_PROIZVODI_OK' )
	  ) uk
	FROM
	(
		SELECT
	...
	(
			...
			Where
			  ...
			  and d.datum_dok between to_date('01.01.'||to_char(2011),'dd.mm.yyyy')
			                      and sysdate
			  and d.status > 0
			  and sd.k_robe <> 0
	...
	,(Select sdp.proizvod
	  ...
	  Where dp.GODINA = 2011
	    ...
	    and dp.datum_dok between to_date('01.01.'||to_char(2011),'dd.mm.yyyy')
		                     and &dDatDo
	...
	where plan.mag = d.org_deo (+)
	  ...
	  and proizv.proizvod (+)= plan.GOTOV_PR_SIFRA
