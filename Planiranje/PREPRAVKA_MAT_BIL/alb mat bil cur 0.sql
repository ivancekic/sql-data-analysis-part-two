   				...
   				plan.STATUS_PLAN,
				     ...
				     , plan.TIP_PROIZVODA
				     , ROUND(sum(STANJE_ZAL),2)STANJE_ZAL
				     , ROUND(sum(STANJE_NA_DAN),2) STANJE_NA_DAN
				...
				FROM
				(
					SELECT distinct STATUS_PLAN, gotov_pr_sifra, gotov_pr_naziv, gotov_pr_jm, GOTOV_PR_POSGR
				                   , gotov_pr_plan_zeljena_kol, p.tip_proizvoda, plv.ORG_DEO_ID
				...
				(
						...
						Where d.broj_dok>0
						  ...
						  and sd.proizvod in
											(
													SELECT distinct gotov_pr_sifra
													FROM PLANIRANJE_VIEW plv
													WHERE plv.PLAN_CIKLUS_ID= &c_ciklus_id
													  AND plv.PLAN_TIP_ID= &c_tip_id
													  AND plv.PLAN_PERIOD_ID= &c_period_id
