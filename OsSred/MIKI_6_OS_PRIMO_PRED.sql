Select FIRMA, IME_PREZIME, tip_op,PROIZVODJAC,
       (case
        WHEN instr(upper(TIP_OP),'MOBIL') > 0 THEN UPPER(SUBSTR(IDENT_NAZIV, TO_CHAR(INSTR(UPPER(IDENT_NAZIV),SUBSTR(PROIZVODJAC,1,3)))))
        ELSE 'NISTA'
        end
       ) MARKA,
...
From
(
SELECT FIRMA, IME_PREZIME, DECODE(DECODE(PROIZVODJAC,'NOKIA','Mobilni',TIP_OPREME),'Tel+sto+kas bez koda','Telefon', tip_opreme) tip_op,
       PROIZVODJAC, INVENTARSKI_BROJ, ident_naziv,DATUM_ZADUZENJA, FIRMA1
FROM
(
