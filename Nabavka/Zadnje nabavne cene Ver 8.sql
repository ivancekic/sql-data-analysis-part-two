...
--	       and d2.datum_dok=ZNC.MAX_D AND D2.DATUM_UNOSA=ZNC.MAX_DU
...
from dokument d, stavka_dok sd, proizvod p, organizacioni_deo od
    ,(
		select a.org_deo, a.proizvod, max(a.max_d) MAX_D, max(a1.max_du) MAX_DU
		from
		(
		           select d1.org_deo, sd1.proizvod, d1.datum_dok MAX_D, d1.datum_unosa MAX_DU
		...
		,
		(
		           select d1.org_deo, sd1.proizvod, max(d1.datum_unosa) MAX_DU
		             from dokument   d1, stavka_dok sd1
		...
		where a.max_du=a1.max_du
        group by a.proizvod, a.org_deo
        order by a.proizvod, a.org_deo
...
,
     (
	    select D2.ORG_DEO, SD2.PROIZVOD, D2.DATUM_DOK, D2.DATUM_UNOSA, sd2.cena, SD2.CENA1, SD2.RABAT, SD2.Z_TROSKOVI
	    from dokument d2, stavka_dok sd2
	    where d2.broj_dok !='0' and d2.vrsta_dok = '3' and d2.godina != 0
	       and d2.tip_dok in ('10','16') and d2.status in ('1','5')
	       and d2.broj_dok  = sd2.broj_dok and d2.vrsta_dok = sd2.vrsta_dok and d2.godina    = sd2.godina
     ) znc1
Where
  -- veza tabela
  ...
  AND d.ORG_DEO   = ZNC1.ORG_DEO
  AND sd.proizvod = ZNC1.PROIZVOD

  AND ZNC.MAX_D  = ZNC1.DATUM_DOK
  AND ZNC.MAX_DU = ZNC1.DATUM_UNOSA
